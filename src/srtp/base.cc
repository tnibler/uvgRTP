...
uvgrtp::base_srtp::~base_srtp()
{
    ...
    if (rekeying_state == OnlyZero || rekeying_state == SwitchingToZero) {
        cleanup_context(remote_srtp_ctxs_[0]);
    }
    if (rekeying_state == OnlyOne || rekeying_state == SwitchingToOne) {
        cleanup_context(remote_srtp_ctxs_[1]);
    }
}
...
std::array<uvgrtp::srtp_ctx_t, 2>& uvgrtp::base_srtp::get_remote_ctxs()
{
    return remote_srtp_ctxs_;
}
...
bool uvgrtp::base_srtp::is_replayed_packet(uint8_t *digest, which_srtp_ctx which_srtp)
{
    const auto& active_srtp = (which_srtp == Zero) ? remote_srtp_ctxs_[0] : remote_srtp_ctxs_[1];
                                    ...
                                    uint8_t* local_salt, uint8_t* remote_salt)
{
    ...
    init_srtp_context(remote_srtp_ctxs_[0], type, rce_flags, remote_key, remote_salt);
...
rtp_error_t uvgrtp::base_srtp::init_srtp_context(uvgrtp::srtp_ctx_t& context, int type, int rce_flags,
...
void uvgrtp::base_srtp::clear_replay_list(which_srtp_ctx which_srtp) {
    if (which_srtp == Zero) {
        replay_lists_[0].clear();
    } else if (which_srtp == One) {
        replay_lists_[1].clear();
    }
}
